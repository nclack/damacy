...
// Recursive rmdir: best-effort cleanup. Only handles flat trees with
// regular files, sufficient for what this test creates.
static void
...
test_meta_cache(void)
{
  ...
  EXPECT(write_file(path, MINIMAL_ZARR_JSON) == 0);
