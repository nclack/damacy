...
// cuStreamDestroy errors if cuFile still holds the stream, so the
// post-destroy returns are the real assertion that set_stream replace
// and detach paths deregister.
static int
