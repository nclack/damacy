...
//
// To unshuffle, output byte at position `off = e*T + bb` is reassembled
// from 8 plane bits: for bp ∈ [0,8), bit bp of the output byte comes
// from plane (bb*8 + bp), bit-position e.
__global__ void
                        ...
                        uint32_t blocksize)
{
  ...
  // Stage 1: copy bitshuffled block into smem.
  ...
  // Stage 2: scatter reassembled bytes to natural layout.
...
#pragma unroll
    for (uint32_t bp = 0; bp < 8; ++bp) {
      const uint8_t plane_byte =
        smem[(bb * 8u + bp) * bytes_per_plane + e_byte];
                        ...
                        uint32_t blocksize)
{
  if (n_ops == 0)
    return 0;
  if (blocksize == 0 || (blocksize & 7u) != 0) {
    log_error("gpu_bitunshuffle_launch: blocksize=%u must be a multiple of 8",
              blocksize);
    return 1;
  }
  static int s_smem_opt_in_done = 0;
  if (!s_smem_opt_in_done) {
    cudaFuncSetAttribute(gpu_bitunshuffle_kernel,
                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                         65536);
    s_smem_opt_in_done = 1;
  }
  if (blocksize > 65536u) {
    log_error("gpu_bitunshuffle_launch: blocksize=%u exceeds 64 KB shmem cap",
              blocksize);
    return 1;
  }
  dim3 grid(DAMACY_BLOSC_MAX_BLOCKS_PER_CHUNK, n_ops, 1);
  gpu_bitunshuffle_kernel<<<grid,
                            kThreadsPerBlock,
                            blocksize,
