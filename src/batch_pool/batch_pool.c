                          ...
                          uint32_t bpe)
{
  ...
  CHECK_SILENT(Invalid, samples_per_batch > 0 && bpe > 0);
  ...
  for (uint8_t d = 0; d < sample_rank; ++d) {
    ...
    CHECK_SILENT(Invalid, spatial_volume <= INT64_MAX / sample_shape[d]);
  ...
  CHECK_SILENT(Invalid,
               (uint64_t)spatial_volume <= INT64_MAX / samples_per_batch);
  uint64_t elements = (uint64_t)samples_per_batch * (uint64_t)spatial_volume;
  if (elements > SIZE_MAX / bpe)
    return DAMACY_BUDGET;
  pool->n_bytes = elements * bpe;
  pool->layout_set = 1;
  return DAMACY_OK;
Rank:
  return DAMACY_RANK;
